...
PanelWindow {
    ...
    function persist() {
        const json = []
        const lua  = ["-- auto-generated by quickshell"]
        for (let i = 0; i < bindsModel.count; i++) {
            ...
            const modStr = it.mods ? it.mods.replace(/,/g, " + ") + " + " : ""
            const escExec = it.exec.replace(/\\/g, "\\\\").replace(/"/g, '\\"')
            lua.push('hl.bind("' + modStr + it.key + '", hl.dsp.exec_cmd("' + escExec + '"))')
        }
        const sJ = JSON.stringify(json).replace(/'/g, "'\\''")
        const sL = lua.join("\n").replace(/'/g, "'\\''")
        saveProc.command = ["sh", "-c",
            "printf '%s' '" + sJ + "' > ~/dotfiles/local/user-keybinds.json && " +
            "printf '%s\\n' '" + sL + "' > ~/dotfiles/local/user-keybinds.lua && " +
    ...
    Process {
        id: bootstrapProc
        command: ["sh", "-c", "mkdir -p ~/dotfiles/local && touch ~/dotfiles/local/user-keybinds.lua && [ -f ~/dotfiles/local/user-keybinds.json ] || echo '[]' > ~/dotfiles/local/user-keybinds.json"]
    }
            ...
            Theme {
                ...
                ColumnLayout {
                    ...
                    RowLayout {
                        ...
                        Repeater {
                            ...
                            delegate: Theme {
                                ...
                                isActive: active
                                isHovered: mMA.containsMouse
                                isPressed: mMA.pressed
                    ...
                    Theme {
                        ...
                        radius: 8
                        style: "button"
                        isHovered: rMA.containsMouse
                        isPressed: rMA.pressed
